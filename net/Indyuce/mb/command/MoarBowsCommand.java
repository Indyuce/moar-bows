...
public class MoarBowsCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (args[0].equalsIgnoreCase("list")) {
			...
			if (!Utils.checkPl(sender, false)) {
				for (Bow b : Bow.values())
					sender.sendMessage("* " + ChatColor.GREEN + " " + b.name);
				return true;
			}
			for (Bow b : Bow.values())
				Json.json((Player) sender, "{\"text\":\"* " + ChatColor.GREEN + ChatColor.translateAlternateColorCodes('&', b.name) + ChatColor.WHITE + ", use /mb get " + b.name() + "\",\"clickEvent\":{\"action\":\"run_command\",\"value\":\"/mb get " + b.name() + "\"},\"hoverEvent\":{\"action\":\"show_text\",\"value\":{\"text\":\"\",\"extra\":[{\"text\":\"Click to get the " + ChatColor.GREEN + ChatColor.translateAlternateColorCodes('&', b.name) + ChatColor.WHITE + ".\",\"color\":\"white\"}]}}}");
		}
		...
		if (args[0].equalsIgnoreCase("get")) {
			...
			sender.sendMessage(ChatColor.YELLOW + t.getName() + " was given " + ChatColor.WHITE + ChatColor.translateAlternateColorCodes('&', bow.name) + ChatColor.YELLOW + ".");

			// message
			String message = Utils.msg("receive-bow");
			if (!ChatColor.stripColor(message).equals("") && sender != t)
				t.sendMessage(ChatColor.YELLOW + message.replace("%bow%", ChatColor.translateAlternateColorCodes('&', bow.name)));
